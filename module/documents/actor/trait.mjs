 ...
 * @param {string} trait   Trait as defined in `CONFIG.DND5R.traits`.
 * @returns {object|void}
 */
...
export function actorKeyPath(trait) {
  const traitConfig = CONFIG.DND5R.traits[trait];
 ...
 * @param {string} [trait]  Trait as defined in `CONFIG.DND5R.traits`, only needed if key isn't prefixed.
 * @returns {string|void}
 */
...
export async function categories(trait) {
  const traitConfig = CONFIG.DND5R.traits[trait];
  const config = foundry.utils.deepClone(CONFIG.DND5R[traitConfig.configKey ?? trait]);
  ...
  for ( const key of Object.keys(config) ) {
    ...
    if ( traitConfig.children?.[key] ) {
      ...
      for ( const [childKey, value] of Object.entries(CONFIG.DND5R[traitConfig.children[key]]) ) {
  ...
  if ( traitConfig.subtypes ) {
    const map = CONFIG.DND5R[`${trait}ProficienciesMap`];

    // Merge all ID lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      foundry.utils.mergeObject(obj, CONFIG.DND5R[key] ?? {});
      return obj;
    }, {});
...
export function getBaseItemUUID(identifier) {
  ...
  let pack = CONFIG.DND5R.sourcePacks.ITEMS;
...
export function traitIndexFields() {
  ...
  for ( const traitConfig of Object.values(CONFIG.DND5R.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  if ( !traitConfig ) return game.i18n.localize(`DND5R.TraitGenericPlural.${pluralRule}`);
 ...
 * @param {string} [config.trait]   Trait as defined in `CONFIG.DND5R.traits` if not using a prefixed key.
...
export function keyLabel(key, config={}) {
  ...
  const traitData = CONFIG.DND5R[traitConfig.configKey ?? trait] ?? {};
  ...
  if ( lastKey === "*" ) {
    ...
    const localization = `DND5R.TraitConfigChoose${final ? "Other" : `Any${count ? "Counted" : "Uncounted"}`}`;
    ...
    for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
      const childLabel = CONFIG.DND5R[childrenKey]?.[lastKey];
      if ( childLabel ) return childLabel;
    }

    // Base item (e.g. "Shortsword")
    for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
      const baseItemId = CONFIG.DND5R[idsKey]?.[lastKey];
...
export function choiceLabel(choice, { only=false, final=false }={}) {
  ...
  return game.i18n.format("DND5R.TraitConfigChooseList", {
    count: choice.count,
...
export function localizedList({ grants=new Set(), choices=[] }) {
  ...
  return game.i18n.format("DND5R.TraitConfigChooseWrapper", {
