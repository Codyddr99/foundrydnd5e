...
export default class CheckActivity extends ActivityMixin(CheckActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "DND5R.CHECK"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "check",
      img: "systems/dnd5r/icons/svg/activity/check.svg",
      title: "DND5R.CHECK.Title",
  ...
  _usageChatButtons(message) {
    ...
    const createButton = (abilityKey, associated) => {
      const ability = CONFIG.DND5R.abilities[abilityKey]?.label;
      const checkType = (associated in CONFIG.DND5R.skills) ? "skill"
        : (associated in CONFIG.DND5R.toolIds) ? "tool": "ability";
      ...
      let label = ability;
      let type;
      if ( checkType === "skill" ) type = CONFIG.DND5R.skills[associated]?.label;
      else if ( checkType === "tool" ) type = Trait.keyLabel(associated, { trait: "tool" });
      if ( type ) label = game.i18n.format("EDITOR.DND5R.Inline.SpecificCheck", { ability, type });
      else label = ability;

      buttons.push({
        label: dc ? `
          <span class="visible-dc">${game.i18n.format("EDITOR.DND5R.Inline.DC", { dc, check: wrap(label) })}</span>
          <span class="hidden-dc">${wrap(label)}</span>
        ` : wrap(label),
        icon: checkType === "tool" ? '<i class="fa-solid fa-hammer" inert></i>'
          : '<i class="dnd5r-icon" data-src="systems/dnd5r/icons/svg/ability-score-improvement.svg" inert></i>',
        dataset
      });
    };
    const wrap = check => game.i18n.format("EDITOR.DND5R.Inline.CheckShort", { check });
  ...
  static async #rollCheck(event, target, message) {
    ...
    if ( !targets.length ) ui.notifications.warn("DND5R.ActionWarningNoToken", { localize: true });
    let { ability, dc, skill, tool } = target.dataset;
    dc = parseInt(dc);
    const rollData = { event, target: Number.isFinite(dc) ? dc : this.check.dc.value };
    if ( ability in CONFIG.DND5R.abilities ) rollData.ability = ability;
