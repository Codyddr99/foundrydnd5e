...
export default class OrderActivity extends ActivityMixin(OrderActivityData) {
  ...
  static metadata = Object.freeze(foundry.utils.mergeObject(super.metadata, {
    type: "order",
    img: "systems/dnd5r/icons/svg/activity/order.svg",
    title: "DND5R.FACILITY.Order.Issue",
    usage: {
      actions: {
        pay: OrderActivity.#onPayOrder
      },
      chatCard: "systems/dnd5r/templates/chat/order-activity-card.hbs",
  ...
  _prepareUsageScaling(usageConfig, messageConfig, item) {
    ...
    messageConfig.data.flags.dnd5r.order = { costs, craft, trade };
  }
  ...
  _usageChatButtons(message) {
    const { costs } = message.data.flags.dnd5r.order;
    if ( !costs.gold || costs.paid ) return [];
    return [{
      label: game.i18n.localize("DND5R.FACILITY.Costs.Automatic"),
      icon: '<i class="fas fa-coins"></i>',
      dataset: { action: "pay", method: "automatic" }
    }, {
      label: game.i18n.localize("DND5R.FACILITY.Costs.Manual"),
  ...
  async _usageChatContext(message) {
    const { costs, craft, trade } = message.data.flags.dnd5r.order;
    const { type } = this.item.system;
    const supplements = [];
    if ( costs.days ) supplements.push(`
      <strong>${game.i18n.localize("DND5R.DurationTime")}</strong>
      ${game.i18n.format("DND5R.FACILITY.Costs.Days", { days: costs.days })}
    `);
    if ( costs.gold ) supplements.push(`
      <strong>${game.i18n.localize("DND5R.CurrencyGP")}</strong>
      ${formatNumber(costs.gold)}
      (${game.i18n.localize(`DND5R.FACILITY.Costs.${costs.paid ? "Paid" : "Unpaid"}`)})
    `);
    if ( craft?.item ) {
    ...
    if ( trade?.stock?.value && trade.sell ) supplements.push(`
      <strong>${game.i18n.localize("DND5R.FACILITY.Trade.Sell.Supplement")}</strong>
      ${formatNumber(trade.stock.value)}
      ${CONFIG.DND5R.currencies.gp?.abbreviation ?? ""}
    `);
    if ( trade?.creatures ) {
      const creatures = [];
      if ( trade.sell ) {
      ...
      supplements.push(`
        <strong>${game.i18n.localize(`DND5R.FACILITY.Trade.${trade.sell ? "Sell" : "Buy"}.Supplement`)}</strong>
        ${game.i18n.getListFormatter({ style: "narrow" }).format(creatures.map(a => a.toAnchor().outerHTML))}
      `);
    }
    const facilityType = game.i18n.localize(`DND5R.FACILITY.Types.${type.value.titleCase()}.Label.one`);
    const buttons = this._usageChatButtons(message);
    return {
      supplements,
      buttons: buttons.length ? buttons : null,
      description: game.i18n.format("DND5R.FACILITY.Use.Description", {
        order: game.i18n.localize(`DND5R.FACILITY.Orders.${this.order}.inf`),
  ...
  static async #onPayOrder(event, target, message) {
    ...
    const order = message.getFlag("dnd5r", "order");
    const config = foundry.utils.expandObject({ "data.flags.dnd5r.order": order });
    if ( method === "automatic" ) {
      try {
    ...
    foundry.utils.setProperty(config, "data.flags.dnd5r.order.costs.paid", true);
