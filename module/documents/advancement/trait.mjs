...
export default class TraitAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      title: game.i18n.localize("DND5R.AdvancementTraitTitle"),
      hint: game.i18n.localize("DND5R.AdvancementTraitHint"),
      apps: {
        config: TraitConfig,
        flow: TraitFlow
      }
  ...
  sortingValueForLevel(levels) {
    const traitOrder = Object.keys(CONFIG.DND5R.traits).findIndex(k => k === this.representedTraits().first());
    const modeOrder = Object.keys(CONFIG.DND5R.traitModes).findIndex(k => k === this.configuration.mode);
  ...
  async actorSelected() {
    ...
    const traitTypes = this.configuration.mode === "default" ? Object.keys(CONFIG.DND5R.traits).filter(k => k !== "dm")
      : filteredKeys(CONFIG.DND5R.traits, t => t[this.configuration.mode === "mastery" ? "mastery" : "expertise"]);
  ...
  async availableChoices(chosen) {
    ...
    if ( this.configuration.allowReplacements && (unfilteredLength > available.length) ) {
      ...
      if ( rep.size === 1 ) return {
        ...
        label: game.i18n.format("DND5R.AdvancementTraitChoicesRemaining", {
