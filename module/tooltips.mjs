...
export default class Tooltips5e {
  ...
  async _onTooltipActivate() {
    ...
    if ( loading?.dataset.uuid ) {
      ...
      if ( doc instanceof dnd5r.documents.Actor5e ) return this._onHoverActor(doc);
      return this._onHoverContentLink(doc);
    }
  ...
  async _onHoverPassive(skill, ability, dc) {
    const skillConfig = CONFIG.DND5R.skills[skill];
    const abilityConfig = CONFIG.DND5R.abilities[ability ?? skillConfig.ability];

    let label;
    if ( skillConfig ) {
      label = game.i18n.format("DND5R.SkillPassiveSpecificHint", { skill: skillConfig.label, ability: abilityConfig.label });
    } else {
      ...
      label = game.i18n.format("DND5R.SkillPassiveHint", { skill: abilityConfig.label });
    }

    const party = game.settings.get("dnd5r", "primaryParty")?.actor;
    if ( !party ) {
    ...
    this.tooltip.classList.add("dnd5r-tooltip", "passive-tooltip");
    this.tooltip.innerHTML = await renderTemplate("systems/dnd5r/templates/journal/passive-tooltip.hbs", context);
