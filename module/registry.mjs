...
class MessageRegistry {
  ...
  static track(message) {
    const origin = message.getFlag("dnd5r", "originatingMessage");
    const type = message.getFlag("dnd5r", "roll.type");
...
class SpellListRegistry {
   ...
   */
  static get options() {
    return Object.entries(CONFIG.DND5R.spellListTypes).map(([type, group]) => {
   ...
   * @param {string} type        Type of list as defined in `CONFIG.DND5R.spellListTypes`.
...
export class SpellList {
   ...
   */
  get name() {
    return dnd5r.registry[SpellList.#REGISTRIES[this.metadata.type]]?.get(this.metadata.identifier)?.name
      ?? this.metadata.name;
  }
