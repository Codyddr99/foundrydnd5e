...
/**
 * Register package source books from `flags.dnd5r.sourceBooks`.
...
function registerSourceBooks(manifest) {
  if ( !manifest.flags.dnd5r?.sourceBooks ) return;
  Object.assign(CONFIG.DND5R.sourceBooks, manifest.flags.dnd5r.sourceBooks);
  return "source books";
}
...
/**
 * Register package spell lists from `flags.dnd5r.spellLists`.
 * @param {Module|System|World} manifest  Manifest from which to register data.
 * @returns {string|void}                 Description of the data registered.
 */
function registerSpellLists(manifest) {
  if ( foundry.utils.getType(manifest.flags.dnd5r?.spellLists) !== "Array" ) return;
  manifest.flags.dnd5r.spellLists.forEach(uuid => dnd5r.registry.spellLists.register(uuid));
  return "spell lists";
}
