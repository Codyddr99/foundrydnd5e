...
export default class EnchantActivityData extends BaseActivityData {
  ...
  get appliedEnchantments() {
    return dnd5r.registry.enchantments.applied(this.uuid);
  }
  ...
  static transformEffectsData(source, options) {
    ...
    for ( const effect of source.effects ) {
      if ( (effect.type !== "enchantment") && (effect.flags?.dnd5r?.type !== "enchantment") ) continue;
      effects.push({ _id: effect._id, ...(effect.flags?.dnd5r?.enchantment ?? {}) });
      delete effect.flags?.dnd5r?.enchantment;
    }
    return effects;
  }
