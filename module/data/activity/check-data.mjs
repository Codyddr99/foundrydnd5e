...
export default class CheckActivityData extends BaseActivityData {
  ...
  /** @override */
  get ability() {
    if ( this.check.dc.calculation in CONFIG.DND5R.abilities ) return this.check.dc.calculation;
  ...
  static transformTypeData(source, activityData, options) {
    return foundry.utils.mergeObject(activityData, {
      check: {
        ability: source.system.ability ?? Object.keys(CONFIG.DND5R.abilities)[0]
      }
    });
  }
  ...
  getAbility(associated) {
    ...
    if ( associated in CONFIG.DND5R.skills ) return CONFIG.DND5R.skills[associated]?.ability ?? null;
    else if ( associated in CONFIG.DND5R.tools ) {
      if ( (this.item.type === "tool") && this.item.system.ability ) return this.item.system.ability;
      return CONFIG.DND5R.tools[associated]?.ability ?? null;
    }
    return null;
  }
}
