...
export default class DurationField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.duration.scalar = this.duration.units in CONFIG.DND5R.scalarTimePeriods;
    if ( this.duration.scalar ) {
      prepareFormulaValue(this, "duration.value", "DND5R.DURATION.FIELDS.duration.value.label", rollData);
    } else this.duration.value = null;

    if ( labels && this.duration.units ) {
      let duration = CONFIG.DND5R.timePeriods[this.duration.units] ?? "";
      ...
      labels.concentrationDuration = this.properties?.has("concentration")
        ? game.i18n.format("DND5R.ConcentrationDuration", { duration }) : duration;
    }
