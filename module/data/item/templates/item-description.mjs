...
export default class ItemDescriptionTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      description: new SchemaField({
        value: new HTMLField({required: true, nullable: true, label: "DND5R.Description"}),
        chat: new HTMLField({required: true, nullable: true, label: "DND5R.DescriptionChat"})
      }),
  ...
  prepareDescriptionData() {
    const uuid = this.parent.flags.dnd5r?.sourceId ?? this.parent._stats?.compendiumSource ?? this.parent.uuid;
   ...
   */
  get validProperties() {
    return new Set(CONFIG.DND5R.validProperties[this.parent.type] ?? []);
  }
  ...
  static compendiumBrowserPropertiesFilter(type) {
    return {
      label: "DND5R.Properties",
      type: "set",
      config: {
        choices: Object.entries(CONFIG.DND5R.itemProperties).reduce((obj, [k, v]) => {
          if ( CONFIG.DND5R.validProperties[type]?.has(k) ) obj[k] = v;
          return obj;
        }, {}),
