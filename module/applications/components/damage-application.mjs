...
export default class DamageApplicationElement extends TargetedApplicationMixin(ChatTrayElement) {
    ...
    if ( !this.targetList ) {
      ...
      div.innerHTML = `
        <label class="roboto-upper">
          ...
          <span>${game.i18n.localize("DND5R.Apply")}</span>
        ...
        <div class="collapsible-content">
          <div class="wrapper">
            <button class="apply-damage" type="button" data-action="applyDamage">
              ...
              ${game.i18n.localize("DND5R.Apply")}
            </button>
          </div>
        </div>
      `;
  ...
  buildTargetListEntry({ uuid, name }) {
    ...
    const changeSources = types.reduce((acc, {type, change, icon}) => {
      ...
      acc += `
        <button class="change-source unbutton" type="button" data-type="${type}" data-change="${change}"
          ...
          <dnd5r-icon src="${icon}" inert></dnd5r-icon>
    ...
    li.innerHTML = `
      ...
      <div class="calculated temp" data-tooltip="DND5R.HitPointsTemp">
        ${temp}
      </div>
  ...
  getChangeSourceOptions(type, change, options) {
    ...
    let label = game.i18n.format(`DND5R.DamageApplication.Change.${change.capitalize()}`, {
      type: CONFIG.DND5R.damageTypes[type]?.label ?? CONFIG.DND5R.healingTypes[type]?.label
    });
    if ( mode === "ignore" ) label = game.i18n.format("DND5R.DamageApplication.Ignoring", { source: label });
    if ( mode === "downgrade" ) label = game.i18n.format("DND5R.DamageApplication.Downgrading", { source: label });
  ...
  refreshListEntry(token, entry, options) {
    ...
    calculatedDamage.dataset.tooltip = `DND5R.${total < 0 ? "Healing" : "Damage"}`;
