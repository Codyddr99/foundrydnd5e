...
export default class FiligreeBoxElement extends AdoptedStyleSheetMixin(HTMLElement) {
  ...
  static CSS = `
    :host {
      ...
      isolation: isolate;
      min-height: 56px;
      filter: var(--filigree-drop-shadow, drop-shadow(0 0 12px var(--dnd5r-shadow-15)));
    }
    .backdrop {
      --chamfer: 12px;
      position: absolute;
      inset: 0;
      background: var(--filigree-background-color, var(--dnd5r-color-card));
      z-index: -2;
      clip-path: polygon(
        var(--chamfer) 0,
    ...
    .filigree {
      position: absolute;
      fill: var(--filigree-border-color, var(--dnd5r-color-gold));
      z-index: -1;
