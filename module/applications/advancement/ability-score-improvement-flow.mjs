...
export default class AbilityScoreImprovementFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/dnd5r/templates/advancement/ability-score-improvement-flow.hbs"
    });
  }
  ...
  async getData() {
    const points = {
      assigned: Object.keys(CONFIG.DND5R.abilities).reduce((assigned, key) => {
    ...
    const abilities = Object.entries(CONFIG.DND5R.abilities).reduce((obj, [key, data]) => {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      pointCap: game.i18n.format(
        `DND5R.ADVANCEMENT.AbilityScoreImprovement.CapDisplay.${pluralRules.select(points.cap)}`, {points: points.cap}
      ),
      pointsRemaining: game.i18n.format(
        `DND5R.ADVANCEMENT.AbilityScoreImprovement.PointsRemaining.${pluralRules.select(points.available)}`,
  ...
  async _onDrop(event) {
    ...
    if ( (item.type !== "feat") || (item.system.type.value !== "feat") ) {
      ui.notifications.error("DND5R.ADVANCEMENT.AbilityScoreImprovement.Warning.Type", {localize: true});
      return null;
    }
    ...
    if ( (item.system.prerequisites?.level ?? -Infinity) > this.advancement.actor.system.details.level ) {
      ui.notifications.error(game.i18n.format("DND5R.ADVANCEMENT.AbilityScoreImprovement.Warning.Level", {
