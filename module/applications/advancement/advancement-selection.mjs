...
export default class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["dnd5r", "sheet", "advancement"],
      template: "systems/dnd5r/templates/advancement/advancement-selection.hbs",
      title: "DND5R.AdvancementSelectionTitle",
      width: 500,
      height: "auto"
    });
  }
  ...
  getData() {
    const context = { types: {} };
    for ( let [name, config] of Object.entries(CONFIG.DND5R.advancementTypes) ) {
    ...
    context.types = dnd5r.utils.sortObjectEntries(context.types, "label");
    return context;
  }
  ...
  static async createDialog(item, { rejectClose=false, options={} }={}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize("DND5R.AdvancementSelectionTitle")}: ${item.name}`,
        buttons: {
          submit: {
            callback: html => {
