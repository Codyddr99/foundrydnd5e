...
export default class StartingEquipmentConfig extends DocumentSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["dnd5r", "starting-equipment"],
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("DND5R.StartingEquipment.Action.Configure")}: ${this.document.name}`;
  }
  ...
  async _onDrop(event) {
    ...
    if ( !item.system.constructor._schemaTemplates?.includes(PhysicalItemTemplate) ) {
      ui.notifications.error(game.i18n.format("DND5R.StartingEquipment.Warning.ItemTypeInvalid", {
  ...
  _onSortEntry(event, data) {
    ...
    if ( dropEntry?.type in EquipmentEntryData.GROUPING_TYPES ) {
      ...
      if ( depth > 3 ) {
        ui.notifications.warn("DND5R.StartingEquipment.Warning.Depth", { localize: true });
        return;
      }
