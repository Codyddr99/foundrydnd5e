...
export default function ActorSheetV2Mixin(Base) {
  return class ActorSheetV2 extends DocumentSheetV2Mixin(Base) {
    constructor(object, options={}) {
      ...
      const { width, height } = game.user.getFlag("dnd5r", `sheetPrefs.${key}`) ?? {};
    ...
    async _renderOuter() {
      ...
      nav.append(...this.constructor.TABS.map(({ tab, label, icon, svg }) => {
        ...
        else if ( svg ) item.innerHTML = `<dnd5r-icon src="systems/dnd5r/icons/svg/${svg}.svg"></dnd5r-icon>`;
        return item;
      }));
    ...
    async getData(options) {
      ...
      context.sidebarCollapsed = !!game.user.getFlag("dnd5r", `${sheetPrefs}.collapseSidebar`);
      if ( context.sidebarCollapsed ) context.cssClass += " collapsed";
      const { attributes } = this.actor.system;

      // Portrait
      const showTokenPortrait = this.actor.getFlag("dnd5r", "showTokenPortrait") === true;
      ...
      ["success", "failure"].forEach(deathSave => {
        ...
        for ( let i = 1; i < 4; i++ ) {
          ...
          const i18nKey = `DND5R.DeathSave${deathSave.titleCase()}Label`;
      ...
      // Senses
      context.senses = Object.entries(CONFIG.DND5R.senses).reduce((obj, [k, label]) => {
      ...
      context.conditions = Object.entries(CONFIG.DND5R.conditionTypes).reduce((arr, [k, c]) => {
        if ( c.pseudo ) return arr; // Filter out pseudo-conditions.
        const { label: name, icon, reference } = c;
        const id = staticID(`dnd5r${k}`);
        conditionIds.add(id);
      ...
      for ( const category of Object.values(context.effects) ) {
        category.effects = await category.effects.reduce(async (arr, effect) => {
          ...
          if ( (source instanceof dnd5r.documents.ActiveEffect5e) && (source.target !== this.object) ) {
          ...
          arr.push({
            ...
            hasTooltip: source instanceof dnd5r.documents.Item5e
          });
          return arr;
        }, []);
      }
    ...
    _prepareTraits() {
      const traits = {};
      for ( const [trait, config] of Object.entries(CONFIG.DND5R.traits) ) {
        ...
        values = values.map(key => {
          ...
          if ( data.bypasses?.size && CONFIG.DND5R.damageTypes[key]?.isPhysical ) icons.push(...data.bypasses.map(p => {
            const type = CONFIG.DND5R.itemProperties[p]?.label;
            return { icon: p, label: game.i18n.format("DND5R.DamagePhysicalBypassesShort", { type }) };
          }));
          return value;
        });
      ...
      if ( this.document.hasConditionEffect("petrification") ) {
        traits.dr = [{ label: game.i18n.localize("DND5R.DamageAll") }];
      }
      ...
      if ( dm ) {
        ...
        const values = Object.entries(dm.amount).map(([k, v]) => {
          ...
          const value = {
            label: `${CONFIG.DND5R.damageTypes[k]?.label ?? k} ${formatNumber(total, { signDisplay: "always" })}`,
            color: total > 0 ? "maroon" : "green"
          };
          const icons = value.icons = [];
          if ( dm.bypasses.size && CONFIG.DND5R.damageTypes[k]?.isPhysical ) icons.push(...dm.bypasses.map(p => {
            const type = CONFIG.DND5R.itemProperties[p]?.label;
            return { icon: p, label: game.i18n.format("DND5R.DamagePhysicalBypassesShort", { type }) };
          }));
          return value;
      ...
      for ( const key of this.actor.system.traits?.weaponProf?.mastery?.value ?? [] ) {
        ...
        value.icons.push({ icon: "mastery", label: game.i18n.format("DND5R.WEAPON.Mastery.Label") });
      }
    ...
    _prepareItems(context) {
      ...
      context.spellbook.forEach(section => {
        section.categories = [
          { activityPartial: "dnd5r.activity-column-school" },
          { activityPartial: "dnd5r.activity-column-time" },
          { activityPartial: "dnd5r.activity-column-range" },
          { activityPartial: "dnd5r.activity-column-target" },
        ...
        section.pips = Array.fromRange(max, 1).map(n => {
          ...
          const label = filled
            ? game.i18n.format(`DND5R.SpellSlotN.${plurals.select(n)}`, { n })
            : game.i18n.localize("DND5R.SpellSlotExpended");
          const classes = ["pip"];
          if ( filled ) classes.push("filled");
    ...
    _prepareItem(item, ctx) {
      ...
      if ( item.type === "spell" ) {
        ...
        const abbr = {
          action: "DND5R.ActionAbbr",
          bonus: "DND5R.BonusActionAbbr",
          reaction: "DND5R.ReactionAbbr",
          minute: "DND5R.TimeMinuteAbbr",
          hour: "DND5R.TimeHourAbbr",
          day: "DND5R.TimeDayAbbr"
        ...
        if ( units && (units !== "none") ) {
          if ( units in CONFIG.DND5R.movementUnits ) {
            ctx.range = {
              distance: true,
              value: system.range.value,
              unit: game.i18n.localize(`DND5R.Dist${units.capitalize()}Abbr`)
            };
          }
        ...
        if ( config.prepares && !linked ) {
          ...
          ctx.preparation = {
            ...
            title: isAlways
              ? CONFIG.DND5R.spellPreparationModes.always.label
              : prepared
                ? CONFIG.DND5R.spellPreparationModes.prepared.label
                : game.i18n.localize("DND5R.SpellUnprepared")
          };
        }
      ...
      // Gear
        ...
        if ( "equipped" in system ) {
          ctx.equip = {
            ...
            title: `DND5R.${system.equipped ? "Equipped" : "Unequipped"}`,
      ...
      ctx.save.ability = ctx.save.ability?.size ? ctx.save.ability.size === 1
        ? CONFIG.DND5R.abilities[ctx.save.ability.first()]?.abbreviation
        : game.i18n.localize("DND5R.AbbreviationDC") : null;

      // Activities
      ctx.activities = item.system.activities
        ?.filter(a => !item.getFlag("dnd5r", "riders.activity")?.includes(a.id))
      ...
      return {
        ...
        save: save ? {
          ...
          ability: save.ability?.size
            ? save.ability.size === 1
              ? CONFIG.DND5R.abilities[save.ability.first()]?.abbreviation
    ...
    _getLabels() {
      ...
      labels.damageAndHealing = { ...CONFIG.DND5R.damageTypes, ...CONFIG.DND5R.healingTypes };
      return labels;
    }
    ...
    _onChangeTab(event, tabs, active) {
      ...
      const sidebarCollapsed = game.user.getFlag("dnd5r", `${sheetPrefs}.collapseSidebar`);
    ...
    _onCreateChild() {
      ...
      if ( activeTab === "effects" ) return ActiveEffect.implementation.create({
        name: game.i18n.localize("DND5R.EffectNew"),
    ...
    _onResize(event) {
      ...
      game.user.setFlag("dnd5r", `sheetPrefs.${key}`, { width, height });
    }
    ...
    _onToggleSidebar() {
      ...
      game.user.setFlag("dnd5r", `sheetPrefs.${this.actor.type}.tabs.${activeTab}.collapseSidebar`, collapsed);
    }
