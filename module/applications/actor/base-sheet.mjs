...
export default class ActorSheet5e extends ActorSheetMixin(ActorSheet) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      scrollY: [
        "dnd5r-inventory .inventory-list",
        "dnd5r-effects .effects-list",
        ".center-pane"
      ],
      tabs: [{navSelector: ".tabs", contentSelector: ".sheet-body", initial: "description"}],
      width: 720,
      height: Math.max(680, Math.max(
        237 + (Object.keys(CONFIG.DND5R.abilities).length * 70),
        240 + (Object.keys(CONFIG.DND5R.skills).length * 24)
      )),
      elements: {
        effects: "dnd5r-effects",
  ...
  /** @override */
  get template() {
    if ( !game.user.isGM && this.actor.limited ) return "systems/dnd5r/templates/actors/limited-sheet.hbs";
    return `systems/dnd5r/templates/actors/${this.actor.type}-sheet.hbs`;
  }
  ...
  async getData(options) {
    ...
    for ( const [a, abl] of Object.entries(context.abilities) ) {
      ...
      abl.hover = CONFIG.DND5R.proficiencyLevels[abl.proficient];
      abl.label = CONFIG.DND5R.abilities[a]?.label;
    ...
    const baseAbility = (prop, key) => {
      ...
      if ( src ) return src;
      if ( prop === "skills" ) src = CONFIG.DND5R.skills[key]?.ability;
      return src ?? "int";
    };
    ["skills", "tools"].forEach(prop => {
      for ( const [key, entry] of Object.entries(context[prop]) ) {
        entry.abbreviation = CONFIG.DND5R.abilities[entry.ability]?.abbreviation;
        entry.icon = this._getProficiencyIcon(entry.value);
        entry.hover = CONFIG.DND5R.proficiencyLevels[entry.value];
        entry.label = (prop === "skills") ? CONFIG.DND5R.skills[key]?.label : Trait.keyLabel(key, {trait: "tool"});
  ...
  _getLabels() {
    ...
    // Currency Labels
    labels.currencies = Object.entries(CONFIG.DND5R.currencies).reduce((obj, [k, c]) => {
      obj[k] = c.label;
      return obj;
    }, {});

    // Proficiency
    labels.proficiency = game.settings.get("dnd5r", "proficiencyModifier") === "dice"
  ...
  _getMovementSpeed(systemData, largestPrimary=false) {
    ...
    let speeds = [
      [movement.burrow, `${game.i18n.localize("DND5R.MovementBurrow")} ${movement.burrow}`],
      [movement.climb, `${game.i18n.localize("DND5R.MovementClimb")} ${movement.climb}`],
      [movement.fly, `${game.i18n.localize("DND5R.MovementFly")} ${movement.fly}${movement.hover ? ` (${game.i18n.localize("DND5R.MovementHover")})` : ""}`],
      [movement.swim, `${game.i18n.localize("DND5R.MovementSwim")} ${movement.swim}`]
    ];
    if ( largestPrimary ) {
      speeds.push([movement.walk, `${game.i18n.localize("DND5R.MovementWalk")} ${movement.walk}`]);
    }
    ...
    if ( largestPrimary ) {
      ...
      return {
        primary: `${primary ? primary[1] : "0"} ${movement.units || Object.keys(CONFIG.DND5R.movementUnits)[0]}`,
    ...
    // Case 2: Walk as primary
      ...
      return {
        primary: `${movement.walk || 0} ${movement.units || Object.keys(CONFIG.DND5R.movementUnits)[0]}`,
  ...
  _getSenses(systemData) {
    ...
    const tags = {};
    for ( let [k, label] of Object.entries(CONFIG.DND5R.senses) ) {
      const v = senses[k] ?? 0;
      if ( v === 0 ) continue;
      tags[k] = `${game.i18n.localize(label)} ${v} ${senses.units ?? Object.keys(CONFIG.DND5R.movementUnits)[0]}`;
    }
  ...
  _prepareTraits(systemData) {
    const traits = {};
    for ( const [trait, traitConfig] of Object.entries(CONFIG.DND5R.traits) ) {
      ...
      if ( data.bypasses?.size ) {
        values = values.filter(t => {
          if ( !CONFIG.DND5R.damageTypes[t]?.isPhysical ) return true;
          physical.push(t);
          return false;
        });
      }
      ...
      if ( physical.length ) {
        ...
        data.selected.physical = game.i18n.format("DND5R.DamagePhysicalBypasses", {
          ...
          bypassTypes: bypassFormatter.format(data.bypasses.reduce((acc, t) => {
            const v = CONFIG.DND5R.itemProperties[t];
      ...
      if ( (trait === "dr") && this.document.hasConditionEffect("petrification") ) {
        data.selected = { custom1: game.i18n.localize("DND5R.DamageAll") };
        data.cssClass = "";
      }
    }
    return traits;
  ...
  _prepareSpellbook(context, spells) {
    ...
    const sections = Object.entries(CONFIG.DND5R.spellPreparationModes).reduce((acc, [k, {order}]) => {
    ...
    const maxLevel = Array.fromRange(Object.keys(CONFIG.DND5R.spellLevels).length - 1, 1).reduce((max, i) => {
    ...
    if ( maxLevel > 0 ) {
      registerSection("spell0", 0, CONFIG.DND5R.spellLevels[0]);
      for (let lvl = 1; lvl <= maxLevel; lvl++) {
        const sl = `spell${lvl}`;
        registerSection(sl, lvl, CONFIG.DND5R.spellLevels[lvl], levels[sl]);
      }
    }

    // Create spellbook sections for all alternative spell preparation modes that have spell slots.
    for ( const [k, v] of Object.entries(CONFIG.DND5R.spellPreparationModes) ) {
      if ( !(k in levels) || !v.upcast || !levels[k].max ) continue;

      if ( !spellbook["0"] && v.cantrips ) registerSection("spell0", 0, CONFIG.DND5R.spellLevels[0]);
      const l = levels[k];
      const level = game.i18n.localize(`DND5R.SpellLevel${l.level}`);
    ...
    spells.forEach(spell => {
      ...
      // Spells from items
      if ( spell.getFlag("dnd5r", "cachedFor") ) {
        s = "item";
        if ( !spell.system.linkedActivity?.displayInSpellbook ) return;
        if ( !spellbook[s] ) registerSection(null, s, game.i18n.localize("DND5R.CAST.SECTIONS.Spellbook"));
      }
      ...
      // Specialized spellcasting modes (if they exist)
        ...
        if ( !spellbook[s] ) {
          const l = levels[mode] || {};
          const config = CONFIG.DND5R.spellPreparationModes[mode];
      ...
      // Sections for higher-level spells which the caster "should not" have, but spell items exist for
        ...
        registerSection(sl, s, CONFIG.DND5R.spellLevels[s], {levels: levels[sl]});
      }
  ...
  _filterItems(items, filters) {
    ...
    const spellSchools = new Set(Object.keys(CONFIG.DND5R.spellSchools));
   ...
   * @param {number} level  A proficiency mode defined in `CONFIG.DND5R.proficiencyLevels`.
    ...
    for ( const override of Object.keys(foundry.utils.flattenObject(this.actor.overrides)) ) {
      ...
      for ( const [key, regex] of Object.entries(proficiencyToggles) ) {
        ...
        if ( match ) {
          ...
          toggle.attr("data-tooltip", "DND5R.ActiveEffectOverrideWarning");
        }
      }

      const [, spell] = override.match(/system\.spells\.(spell\d)\.override/) || [];
      if ( spell ) {
        html.find(`.spell-max[data-level="${spell}"]`).attr("data-tooltip", "DND5R.ActiveEffectOverrideWarning");
      }
    }
  }
  ...
  async _onDropActor(event, data) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("dnd5r", "allowPolymorphing"));
    ...
    const rememberOptions = html => {
      ...
      const settings = foundry.utils.mergeObject(game.settings.get("dnd5r", "polymorphSettings") ?? {}, options);
      game.settings.set("dnd5r", "polymorphSettings", settings);
      return settings;
    };

    // Create and render the Dialog
    return new Dialog({
      title: game.i18n.localize("DND5R.PolymorphPromptTitle"),
      content: {
        options: game.settings.get("dnd5r", "polymorphSettings"),
        settings: CONFIG.DND5R.polymorphSettings,
        effectSettings: CONFIG.DND5R.polymorphEffectSettings,
      ...
      buttons: {
        accept: {
          ...
          label: game.i18n.localize("DND5R.PolymorphAcceptSettings"),
          callback: html => this.actor.transformInto(sourceActor, rememberOptions(html))
        },
        wildshape: {
          icon: CONFIG.DND5R.transformationPresets.wildshape.icon,
          label: CONFIG.DND5R.transformationPresets.wildshape.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.DND5R.transformationPresets.wildshape.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        polymorph: {
          icon: CONFIG.DND5R.transformationPresets.polymorph.icon,
          label: CONFIG.DND5R.transformationPresets.polymorph.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.DND5R.transformationPresets.polymorph.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        self: {
          icon: CONFIG.DND5R.transformationPresets.polymorphSelf.icon,
          label: CONFIG.DND5R.transformationPresets.polymorphSelf.label,
          callback: html => this.actor.transformInto(sourceActor, foundry.utils.mergeObject(
            CONFIG.DND5R.transformationPresets.polymorphSelf.options,
            { transformTokens: rememberOptions(html).transformTokens }
          ))
        },
        cancel: {
      ...
      classes: ["dialog", "dnd5r", "polymorph"],
      width: 900,
      template: "systems/dnd5r/templates/apps/polymorph-prompt.hbs"
    }).render(true);
  }
  ...
  async _onDropItemCreate(itemData, event) {
    ...
    if ( multipleAdvancements && !game.settings.get("dnd5r", "disableAdvancements") ) {
      ui.notifications.warn(game.i18n.format("DND5R.WarnCantAddMultipleAdvancements"));
  ...
  async _onDropSingleItem(itemData, event) {
    ...
    if ( this.constructor.unsupportedItemTypes.has(itemData.type) ) {
      ui.notifications.warn(game.i18n.format("DND5R.ActorWarningInvalidItem", {
    ...
    if ( this.actor.system.metadata?.supportsAdvancement && itemData.system.advancement?.length
        && !game.settings.get("dnd5r", "disableAdvancements") ) {
      ...
      if ( singleton && this.actor.itemTypes[itemData.type].length ) {
        ui.notifications.error(game.i18n.format("DND5R.ActorWarningSingleton", {
          itemType: game.i18n.localize(CONFIG.Item.typeLabels[itemData.type]),
          actorType: game.i18n.localize(CONFIG.Actor.typeLabels[this.actor.type])
        }));
        return false;
  ...
  _onDropSpell(itemData, event) {
    ...
    const spellcastKeys = Object.keys(CONFIG.DND5R.spellcastingTypes);
  ...
  _getHeaderButtons() {
    ...
    if ( this.actor.isPolymorphed ) {
      buttons.unshift({
        label: "DND5R.PolymorphRestoreTransformation",
