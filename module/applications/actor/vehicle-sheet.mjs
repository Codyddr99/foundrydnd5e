...
export default class ActorSheet5eVehicle extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["dnd5r", "sheet", "actor", "vehicle"]
    });
  }
  ...
  _prepareCrewedItem(item, context) {
    ...
    context.toggleTitle = game.i18n.localize(`DND5R.${isCrewed ? "Crewed" : "Uncrewed"}`);

    // Handle crew actions
    if ( item.type === "feat" && item.system.activation.type === "crew" ) {
      context.cover = game.i18n.localize(`DND5R.${item.system.cover ? "CoverTotal" : "None"}`);
  ...
  _prepareItems(context) {
    const cargoColumns = [{
      label: game.i18n.localize("DND5R.Quantity"),
      css: "item-qty",
      property: "quantity",
      editable: "Number"
    }];
    ...
    const equipmentColumns = [{
      ...
      label: game.i18n.localize("DND5R.AC"),
      css: "item-ac",
      property: "system.armor.value"
    }, {
      label: game.i18n.localize("DND5R.HP"),
      css: "item-hp",
      property: "system.hp.value",
      maxProperty: "system.hp.max",
      editable: "Number"
    }, {
      label: game.i18n.localize("DND5R.Threshold"),
      css: "item-threshold",
      property: "threshold"
    }];
    ...
    const features = {
      actions: {
        label: game.i18n.localize("DND5R.ActionPl"),
        items: [],
        hasActions: true,
        crewable: true,
      ...
      passive: {
        label: game.i18n.localize("DND5R.Features"),
        items: [],
        dataset: {type: "feat"}
      },
      reactions: {
        label: game.i18n.localize("DND5R.ReactionPl"),
        items: [],
    ...
    const cargo = {
      crew: {
        label: game.i18n.localize("DND5R.VehicleCrew"),
      ...
      passengers: {
        label: game.i18n.localize("DND5R.VehiclePassengers"),
      ...
      cargo: {
        label: game.i18n.localize("DND5R.VehicleCargo"),
        items: [],
        dataset: {type: "loot"},
        columns: [{
          label: game.i18n.localize("DND5R.Quantity"),
          ...
          label: game.i18n.localize("DND5R.Price"),
          css: "item-price",
          property: "system.price.value",
          editable: "Number"
        }, {
          label: game.i18n.localize("DND5R.Weight"),
          css: "item-weight",
    ...
    for ( const item of context.items ) {
      ...
      const isCargo = item.flags.dnd5r?.vehicleCargo === true;
      if ( isCargo ) {
  ...
  activateListeners(html) {
    ...
    html[0].querySelector('[data-tab="cargo"] dnd5r-inventory')
  ...
  async _onDropSingleItem(itemData, event) {
    ...
    foundry.utils.setProperty(itemData, "flags.dnd5r.vehicleCargo", isCargo);
