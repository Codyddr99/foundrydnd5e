...
export default class DamagesConfig extends TraitsConfig {
  ...
  static PARTS = {
    traits: {
      template: "systems/dnd5r/templates/actors/config/damages-config.hbs"
    }
  };
  ...
  /** @override */
  get otherLabel() {
    return game.i18n.localize("DND5R.DamageTypes");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    context.bypasses = new SelectChoices(Object.entries(CONFIG.DND5R.itemProperties).reduce((obj, [k, v]) => {
    ...
    if ( this.options.trait === "dm" ) {
      ...
      context.bypassHint = "DND5R.DamageModification.BypassHint";
      context.hint = "DND5R.DamageModification.Hint";
      context.value.field = new FormulaField({ determinstic: true });
      context.value.key = "amount";
    } else {
      context.bypassHint = "DND5R.DamagePhysicalBypassHint";
  ...
  _processChoice(data, key, choice, categoryChosen=false) {
    ...
    const config = CONFIG.DND5R.damageTypes[key];
