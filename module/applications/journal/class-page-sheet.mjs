...
export default class JournalClassPageSheet extends JournalPageSheet {
  ...
  /** @inheritDoc */
  get template() {
    return `systems/dnd5r/templates/journal/page-${this.document.type}-${this.isEditable ? "edit" : "view"}.hbs`;
  }
  ...
  async getData(options) {
    ...
    context.styleOptions = [
      { value: "", label: game.i18n.localize("JOURNALENTRYPAGE.DND5R.Class.Style.Inferred") },
      { rule: true },
      { value: "2024", label: game.i18n.localize("JOURNALENTRYPAGE.DND5R.Class.Style.Modern") },
      { value: "2014", label: game.i18n.localize("JOURNALENTRYPAGE.DND5R.Class.Style.Legacy") }
    ];
    ...
    if ( linked.system.primaryAbility ) {
      ...
      ).format(Array.from(linked.system.primaryAbility.value).map(v => CONFIG.DND5R.abilities[v]?.label));
    }
  ...
  async _getTable(item, { initialLevel=1, modernStyle }={}) {
    ...
    const headers = [[{content: game.i18n.localize("DND5R.Level")}]];
    if ( item.type === "class" ) headers[0].push({content: game.i18n.localize("DND5R.ProficiencyBonus")});
    if ( hasFeatures ) headers[0].push({content: game.i18n.localize("DND5R.Features")});
    ...
    const rows = [];
    for ( const level of Array.fromRange((CONFIG.DND5R.maxLevel - (initialLevel - 1)), initialLevel) ) {
      let features = [];
      for ( const advancement of item.advancement.byLevel[level] ) {
        switch ( advancement.constructor.typeName ) {
          case "AbilityScoreImprovement":
            features.push(game.i18n.localize("DND5R.ADVANCEMENT.AbilityScoreImprovement.Title"));
            continue;
          case "ItemGrant":
  ...
  async _getSpellProgression(item) {
    ...
    if ( spellcasting.type === "leveled" ) {
      const spells = {};
      const maxSpellLevel = CONFIG.DND5R.SPELL_SLOT_TABLE[CONFIG.DND5R.SPELL_SLOT_TABLE.length - 1].length;
      Array.fromRange(maxSpellLevel, 1).forEach(l => spells[`spell${l}`] = {});

      let largestSlot;
      for ( const level of Array.fromRange(CONFIG.DND5R.maxLevel, 1).reverse() ) {
      ...
      table.headers = [
        [{content: game.i18n.localize("JOURNALENTRYPAGE.DND5R.Class.SpellSlotsPerSpellLevel"), colSpan: largestSlot}],
      ...
      table.headers = [[
        { content: game.i18n.localize("JOURNALENTRYPAGE.DND5R.Class.SpellSlots") },
        { content: game.i18n.localize("JOURNALENTRYPAGE.DND5R.Class.SpellSlotLevel") }
      ]];
      table.cols = [{class: "spellcasting", span: 2}];

      // Loop through each level, gathering "Spell Slots" & "Slot Level" for each one
      for ( const level of Array.fromRange(CONFIG.DND5R.maxLevel, 1) ) {
       ...
       * The actual hook names include the spellcasting type (e.g. `dnd5r.buildPsionicSpellcastingTable`).
       ...
       * @function dnd5r.buildSpellcastingTable
       * @memberof hookEvents
       */
      Hooks.callAll(
        `dnd5r.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting
      );
    }
  ...
  async _getOptionalTable(item, { modernStyle }) {
    const headers = [[
      { content: game.i18n.localize("DND5R.Level") },
      { content: game.i18n.localize("DND5R.Features") }
    ]];
  ...
  async _getFeatures(item, { modernStyle, optional=false }) {
    const prepareFeature = async (f, level) => {
      ...
      return {
        document,
        name: modernStyle ? game.i18n.format("JOURNALENTRYPAGE.DND5R.Class.Features.Name", {
