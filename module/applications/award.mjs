...
export default class Award extends DialogMixin(FormApplication) {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["dnd5r2", "award", "dialog"],
      template: "systems/dnd5r/templates/apps/award.hbs",
      title: "DND5R.Award.Title",
      width: 350,
      height: "auto",
      currency: null,
      xp: null,
  ...
  getData(options={}) {
    ...
    context.currency = Object.entries(CONFIG.DND5R.currencies).reduce((obj, [k, { label, icon }]) => {
      obj[k] = {
        label, icon,
    ...
    context.hideXP = game.settings.get("dnd5r", "levelingMode") === "noxp";
    context.noPrimaryParty = !game.settings.get("dnd5r", "primaryParty")?.actor && !this.isPartyAward;
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<dnd5r-icon class="fa-fw" src="systems/dnd5r/icons/svg/backpack.svg"></dnd5r-icon>',
  ...
  _saveDestinations(destinations) {
    ...
    target.setFlag("dnd5r", "awardDestinations", destinations);
  }
  ...
  static async displayAwardMessages(results) {
    ...
    for ( const [destination, result] of results ) {
      ...
      if ( result.xp ) entries.push(`
        <span class="award-entry">
          ${formatNumber(result.xp)} ${game.i18n.localize("DND5R.ExperiencePointsAbbr")}
        </span>
      `);
      if ( !entries.length ) continue;

      const content = game.i18n.format("DND5R.Award.Message", {
        name: destination.name, award: `<span class="dnd5r2">${game.i18n.getListFormatter().format(entries)}</span>`
      });
  ...
  static async handleAward(message) {
    if ( !game.user.isGM ) {
      ui.notifications.error("DND5R.Award.NotGMError", { localize: true });
      return;
    }
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("dnd5r", "awardDestinations");
  ...
  static parseAwardCommand(message) {
    ...
    for ( const part of command.split(" ") ) {
      ...
      try {
        new Roll(amount);
        if ( label in CONFIG.DND5R.currencies ) currency[label] = amount;
    ...
    if ( unrecognized.length ) throw new Error(game.i18n.format("DND5R.Award.UnrecognizedWarning", {
