...
export default class OrderUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    order: {
      template: "systems/dnd5r/templates/activity/order-usage.hbs"
    },
    footer: {
  ...
  _prepareBuildContext(context, options) {
    context.build = {
      choices: CONFIG.DND5R.facilities.sizes,
      field: new StringField({ nullable: false, blank: false, label: "DND5R.FACILITY.FIELDS.size.label" }),
      name: "building.size",
  ...
  _prepareCostsContext(context, { days, gold }) {
    ...
    context.costs = {
      days: {
        field: new NumberField({ nullable: true, integer: true, min: 0, label: "DND5R.TimeDay" }),
        name: "costs.days",
        value: this.config.costs?.days ?? days ?? duration
      },
      gold: {
        field: new NumberField({ nullable: true, integer: true, min: 0, label: "DND5R.CurrencyGP" }),
        name: "costs.gold",
  ...
  async _prepareCraftContext(context, options) {
    ...
    context.craft = {
      legend: game.i18n.localize(`DND5R.FACILITY.Orders.${this.activity.order}.present`),
      item: {
    ...
    if ( this.activity.order === "harvest" ) {
      ...
      context.craft.baseItem = {
        field: new BooleanField({
          label: "DND5R.FACILITY.Craft.BaseItem.Label",
          hint: "DND5R.FACILITY.Craft.BaseItem.Hint"
        }),
  ...
  async _prepareFooterContext(context, options) {
    context.buttons = [{
      ...
      label: "DND5R.FACILITY.Order.Execute"
    }];
    return context;
  }
  ...
  async _prepareOrderContext(context, options) {
    ...
    if ( this.activity.order === "build" ) {
      const { days, value: gold } = CONFIG.DND5R.facilities.sizes.cramped;
      this._prepareBuildContext(context, options);
      this._prepareCostsContext(context, { ...options, days, gold });
      return;
    }

    let { duration } = game.settings.get("dnd5r", "bastionConfiguration");
    if ( (this.activity.order === "craft") || (this.activity.order === "harvest") ) {
      ...
      const config = CONFIG.DND5R.facilities.orders[this.activity.order];
  ...
  async _prepareTradeContext(context, options) {
    ...
    if ( !trade.creatures.max && !trade.stock.max ) {
      context.trade = {
        stocked: {
          field: new BooleanField({
            label: "DND5R.FACILITY.Trade.Stocked.Label",
            hint: "DND5R.FACILITY.Trade.Stocked.Hint"
          }),
      ...
      context.trade = {
        sell: {
          field: new BooleanField({ label: "DND5R.FACILITY.Trade.Sell.Label" }),
          name: "trade.sell",
          value: isSelling
        }
      };
      ...
      if ( trade.stock.max ) {
        ...
        const label = `DND5R.FACILITY.Trade.Stock.${isSelling ? "Sell" : "Buy"}`;
        context.trade.stock = {
        ...
        context.trade.creatures = {
          buy, sell,
          hint: "DND5R.FACILITY.Trade.Creatures.Buy",
          price: {
            field: new NumberField({
              nullable: false, min: 0, integer: true,
              label: "DND5R.FACILITY.Trade.Price.Label",
              hint: "DND5R.FACILITY.Trade.Price.Hint"
            }),
  ...
  _prepareBuildData(submitData) {
    if ( (this.config.building?.size ?? "cramped") !== submitData.building?.size ) {
      const { days, value: gold } = CONFIG.DND5R.facilities.sizes[submitData.building.size];
