...
export default class SummonSheet extends ActivitySheet {
  ...
  async _prepareEffectContext(context) {
    ...
    context.abilityOptions = [
      { value: "", label: this.activity.isSpell ? game.i18n.localize("DND5R.Spellcasting") : "" },
      { rule: true },
      ...Object.entries(CONFIG.DND5R.abilities).map(([value, { label }]) => ({ value, label }))
    ];
    context.creatureSizeOptions = Object.entries(CONFIG.DND5R.actorSizes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureSizes.has(value)
    }));
    context.creatureTypeOptions = Object.entries(CONFIG.DND5R.creatureTypes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureTypes.has(value)
    }));

    context.profileModes = [
      { value: "", label: game.i18n.localize("DND5R.SUMMON.FIELDS.summon.mode.Direct") },
      { value: "cr", label: game.i18n.localize("DND5R.SUMMON.FIELDS.summon.mode.CR") }
    ];
  ...
  _getTabs() {
    ...
    tabs.effect.label = "DND5R.SUMMON.SECTIONS.Summoning";
    ...
    tabs.effect.tabs = this._markTabs({
      profiles: {
        ...
        label: "DND5R.SUMMON.SECTIONS.Profiles"
      },
      changes: {
        id: "changes", group: "effect", icon: "fa-solid fa-sliders",
        label: "DND5R.SUMMON.SECTIONS.Changes"
      }
    });
    return tabs;
  }
